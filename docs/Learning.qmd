...
small_post_meal_data <- select(post_meal_data, OFS.ID, Age, BMI, Group, auc_pg, auc_ins, glykemi)
...
post_meal_data |>
  ...
  rename(
    ...
    p.insulin.minus5 = Insulin..5.OGTT.X
  )
...
post_meal_data |>
  mutate(
    procent_body_fat = (Fat.mass.DXA / Weight) * 100
  ) |>
  filter(procent_body_fat > 30)
```
...
To ungroup data: ad pipe in the very end and choose function 'ungroup'

knitr::kable() (instead of loading the entire package, just uses a function from it. The function makes the table more presentable)

The round function round op to the number you choose in the second argument of the fuction
```{r}
